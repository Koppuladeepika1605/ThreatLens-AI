...
const MOCK_ALERTS = [
  { event_id: 'EVT-DEMO-91', timestamp: new Date(Date.now() - 4 * 60000).toISOString(), source_ip: '10.20.1.4', destination_ip: '10.20.2.9', protocol: 'TCP', attack_type: 'Reconnaissance', risk_score: 91, severity: 'CRITICAL', is_suspicious: true, suspicious_probability: .94, anomaly_score: .88, attack_probability: .92, top_features: [{ feature: 'ct_srv_src', importance: .117, direction: 'increases_risk' }] },
  { event_id: 'EVT-DEMO-68', timestamp: new Date(Date.now() - 19 * 60000).toISOString(), source_ip: '172.16.4.22', destination_ip: '10.20.2.14', protocol: 'UDP', attack_type: 'Generic', risk_score: 68, severity: 'HIGH', is_suspicious: true, suspicious_probability: .88, anomaly_score: .51, attack_probability: .81, top_features: [{ feature: 'sbytes', importance: .091, direction: 'increases_risk' }] },
  { event_id: 'EVT-DEMO-43', timestamp: new Date(Date.now() - 42 * 60000).toISOString(), source_ip: '192.168.4.7', destination_ip: '10.20.2.4', protocol: 'TCP', attack_type: 'Fuzzers', risk_score: 43, severity: 'MEDIUM', is_suspicious: true, suspicious_probability: .71, anomaly_score: .43, attack_probability: .64, top_features: [{ feature: 'rate', importance: .061, direction: 'increases_risk' }] }
];

const MOCK_STATS = { total_traffic: 12842, normal_count: 11931, suspicious_count: 911, critical_count: 38, attack_distribution: { Reconnaissance: 312, Generic: 248, DoS: 176, Exploits: 104, Fuzzers: 71 }, recent_alerts: MOCK_ALERTS };
...
function App() {
  ...
  async function selectAlert(alert) {
    ...
    try {
      ...
      setReport({ event_id: alert.event_id, summary: `${alert.attack_type} activity detected from ${alert.source_ip} to ${alert.destination_ip}.`, attack_type: alert.attack_type, risk_score: alert.risk_score, severity: alert.severity, confidence: Math.max(alert.suspicious_probability || 0, alert.attack_probability || 0), evidence: [`Binary detector confidence: ${percent(alert.suspicious_probability)}.`, `Behavioral anomaly score: ${percent(alert.anomaly_score)}.`, `Attack taxonomy: ${alert.attack_type} (${percent(alert.attack_probability)}).`], reasoning: ['The live agent is unavailable; this is a local fallback summary.', 'No additional evidence was fabricated in demo mode.'], recommended_actions: ['Review the source IP and adjacent traffic.', 'Run the investigation again when the API is available.'] });
  ...
  async function loadDemoEvent() {
    ...
    } catch { setToast('Demo mode is already active'); setDemoMode(true); setStats(MOCK_STATS); setAlerts(MOCK_ALERTS); selectAlert(MOCK_ALERTS[0]); }
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      <header className="topbar"><div><span className="eyebrow">SECURITY OPERATIONS CENTER / 24H VIEW</span><h1>Command center</h1></div><div className="header-actions"><span className={`data-badge ${demoMode ? 'demo' : ''}`}><i /> {demoMode ? 'DEMO DATA' : 'LIVE TELEMETRY'}</span><button className="icon-button" onClick={loadDashboard} aria-label="Refresh dashboard">↻</button><button className="demo-button" onClick={loadDemoEvent}>Load model event <span>+</span></button></div></header>
      ...
      <section id="investigation" className="section investigation-section"><SectionTitle number="04" title="Incident investigation" meta={selected ? `INCIDENT #${selected.event_id}` : 'SELECT AN ALERT TO BEGIN'} />{selected ? <InvestigationPanel alert={selected} report={report} loading={investigating} reviewed={reviewed.has(selected.event_id)} onReviewed={() => { setReviewed((current) => new Set(current).add(selected.event_id)); action('Alert marked as reviewed'); }} onAction={action} /> : <div className="investigation-empty"><div className="crosshair">+</div><h3>Investigation workspace ready</h3><p>Select an alert above to inspect evidence, reasoning, and response guidance.</p></div>}</section>
...
function StatCard({ label, value, suffix, accent, loading }) { return <div className={`stat-card ${accent}`}><span className="stat-label">{label}</span><div className="stat-value">{loading ? <span className="skeleton" /> : (value ?? 0).toLocaleString()} <small>{suffix}</small></div><div className="stat-line" /></div>; }
